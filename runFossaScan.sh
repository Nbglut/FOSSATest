...
# Get all local branches (including remote tracking branches)
...
do
    # Skip the current branch (in case of "HEAD" or any default state)
    if [ "$BRANCH" == "HEAD" ]; then
        continue
    fi
    ...
    # Checkout the branch in a detached HEAD state (without changing local branches)
    git checkout "$BRANCH" || { echo "Failed to checkout branch $BRANCH"; git stash pop; sleep 1; continue; }
    
    # Run FOSSA scan on the branch
    echo "Running FOSSA scan on branch: $BRANCH"
    ...
    if fossa status --json | jq '.scanStatus' | grep -q 'FAILED'; then
        echo "Branch '$BRANCH' has issues!" >> "$ISSUE_BRANCHES"
    else
        echo "Branch '$BRANCH' passed the scan."
    fi
